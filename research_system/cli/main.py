...
Analyzes validation results to identify:
  ...
  - Data gaps that need to be filled
    ...
    parser = subparsers.add_parser(
        "ideate",
        help="Generate new strategy ideas (LLM-powered)",
        description="""
Generate new strategy ideas using LLM-powered multi-persona synthesis.

Three expert personas analyze your workspace and generate novel strategies:
  - Edge Hunter: Timing and micro-structure edges
  - Macro Strategist: Cross-asset regime plays
  - Quant Archaeologist: Rehabilitate failed approaches

Ideas pass through a quality gate (max 3) before being saved as
pending strategies in strategies/pending/.

Use --quick for fast template-based ideation (no LLM required).
        """,
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--quick",
        action="store_true",
        help="Use template-based ideation (no LLM required)"
    )
    parser.add_argument(
        "--max-ideas", "-n",
        type=int,
    ...
    parser = subparsers.add_parser(
        "synthesize",
        help="Run full cross-strategy synthesis",
        description="""
Run full cross-strategy LLM synthesis using 5 specialist personas
plus a synthesis director.

Requires at least one validated strategy in the workspace.
Analyzes patterns across strategies and generates ranked recommendations.

Specialists: portfolio-architect, instrument-specialist, data-integrator,
             regime-strategist, creative-maverick
Director:    synthesis-director (integrates all perspectives)
        """,
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    parser.add_argument(
...
Examples:
  ...
  research walkforward STRAT-001 --params  # Show parameter evolution
    ...
    parser.set_defaults(func=cmd_walkforward)
...
def cmd_verify(args):
    ...
    # Handle --all flag
    if process_all:
        return _cmd_verify_all(workspace, args)
...
def cmd_validate(args):
    ...
    # Handle --all flag
    if process_all:
        return _cmd_validate_all(workspace, args)
...
def cmd_learn(args):
    ...
    # Handle --all flag
    if process_all:
        return _cmd_learn_all(workspace, args)
...
def cmd_ideate(args):
    """Generate new strategy ideas (LLM-powered or template-based)."""
    workspace = get_workspace_from_args(args)
    ...
    dry_run = getattr(args, 'dry_run', False)
    max_ideas = getattr(args, 'max_ideas', 3)

    if quick_mode:
        return _cmd_ideate_quick(workspace, max_ideas, dry_run)
    else:
        return _cmd_ideate_llm(workspace, max_ideas, dry_run)


def _cmd_ideate_quick(workspace, max_ideas, dry_run):
    """Template-based ideation (no LLM required)."""
    from research_system.validation import Ideator, Learner

    print("\nGenerating strategy ideas (template mode)...")
    print("=" * 50)
...
def _cmd_ideate_llm(workspace, max_ideas, dry_run):
    """LLM-powered multi-persona ideation."""
    from research_system.synthesis import SynthesisRunner

    print("\nLLM-Powered Strategy Ideation")
    print("=" * 50)
    ...
    try:
        ...
        print("Falling back to template-based ideation. Use --quick explicitly next time.")
        return _cmd_ideate_quick(workspace, max_ideas, dry_run)

    runner = SynthesisRunner(workspace, llm_client)
    result = runner.ideate(max_ideas=max_ideas)

    if result.offline:
        print("\nNo LLM backend available.")
        print("Falling back to template-based ideation.")
        return _cmd_ideate_quick(workspace, max_ideas, dry_run)

    # Display results
    print(f"\nPersonas run: {', '.join(result.persona_responses.keys())}")
    print(f"Ideas generated: {result.ideas_generated}")
    print(f"Ideas accepted: {result.ideas_accepted}")
    print(f"Ideas rejected: {result.ideas_rejected}")
    ...
    if result.accepted_ideas:
        print(f"\nAccepted ideas:")
        print("-" * 50)
        for idea in result.accepted_ideas:
            print(f"\n  {idea.name} [{idea.confidence} confidence]")
            print(f"    Persona: {idea.persona}")
            print(f"    Hypothesis: {idea.hypothesis[:150]}")
            print(f"    Entry: {idea.entry_logic[:100]}")

    if not dry_run and result.strategy_files:
        print(f"\nSaved {len(result.strategy_files)} strategy(ies):")
    ...
    if result.report_file:
        print(f"\nSynthesis report: {result.report_file}")
...
def cmd_synthesize(args):
    """Run full cross-strategy LLM synthesis."""
    workspace = get_workspace_from_args(args)
    ...
    if result.errors:
        for err in result.errors:
            print(f"\nError: {err}")
        if not result.persona_responses:
            return 1

    # Display persona summaries
    print(f"\nPersonas run: {len(result.persona_responses)}")
    for persona, response in result.persona_responses.items():
        preview = response[:200].replace('\n', ' ')
        print(f"\n  {persona}:")
        print(f"    {preview}...")

    if result.accepted_ideas:
        print(f"\nGenerated {result.ideas_accepted} strategy recommendation(s):")
        for idea in result.accepted_ideas:
            print(f"  - {idea.name} [{idea.confidence}] (from {idea.persona})")

    if result.strategy_files:
        print(f"\nSaved strategies:")
        for path in result.strategy_files:
            print(f"  {path}")
